...
namespace StackEverythingRedux
{
    internal class StackEverythingRedux : Mod
    {
        ...
        private static void PatchHarmony()
        {
            ...
            Patch(nameof(Item.addToStack), typeof(Item), typeof(TackleAddToStackPatches));
            Patch(nameof(SObject.addToStack), typeof(SObject), typeof(TackleAddToStackPatches));
        ...
        private static void Patch(string originalName, Type originalType, Type patchType)
        {
            ...
            if (prefix != null || postfix != null || transpiler != null)
            {
                try
                {
                    ...
                    Log.Trace($"Patched {originalType}::{originalName} with{(prefix == null ? "" : $" {patchType.Name}::{prefix.Name}")}{(postfix == null ? "" : $" {patchType.Name}::{postfix.Name}")}{(transpiler == null ? "" : $" {patchType.Name}::{transpiler.Name}")}");
                }
                catch (Exception e)
                {
